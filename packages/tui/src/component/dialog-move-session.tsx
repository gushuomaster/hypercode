...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    if (directories.loading && !data && !current) return [{ title: t("dialog.moveSession.loading"), value: undefined }]
    const roots = [...(data ?? [])]
    ...
    if (roots.length === 0) return [{ title: t("dialog.moveSession.empty"), value: undefined }]
    ...
    return list.map((item) => {
      ...
      return {
        ...
        titleView: isRemoving ? (
          <span style={{ fg: theme.error }}>{t("dialog.moveSession.deleting", { directory: item.location })}</span>
        ) : deleting ? (
          <span style={{ fg: theme.text }}>{t("dialog.moveSession.confirmDelete", { key: deleteHint() })}</span>
        ) : suffix ? (
          <>
    ...
    if (result.error) {
      ...
      if ("data" in result.error && result.error.data.forceRequired) {
        ...
        const choice = await DialogWorkspaceFileChanges.show(dialog, status?.data ?? [], {
          title: t("dialog.moveSession.deleteWorkingCopy"),
          message: t("dialog.moveSession.deleteWorkingCopyMessage"),
        })
        if (choice !== "yes") {
          reopen()
          return
        ...
        if (forced.error) {
          toast.show({
            variant: "error",
            title: t("dialog.moveSession.deleteFailed"),
          ...
          showError() ? (
            <box paddingLeft={4} paddingRight={4}>
              <text fg={theme.error} attributes={TextAttributes.BOLD}>
                {t("dialog.moveSession.loadError")}
              </text>
