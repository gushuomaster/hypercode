...
export function DialogExportOptions(props: DialogExportOptionsProps) {
  ...
  useBindings(() => ({
    bindings: [
      {
        key: "tab",
        desc: t("dialog.export.nextOption"),
        group: t("dialog.category"),
        cmd: () => {
  ...
  useBindings(() => ({
    ...
    bindings: [
      {
        key: "space",
        desc: t("dialog.export.toggleOption"),
        group: t("dialog.category"),
        cmd: () => {
  ...
  return (
    <box paddingLeft={2} paddingRight={2} gap={1}>
      ...
      <box gap={1}>
        <box>
          <text fg={theme.text}>{t("dialog.export.filename")}</text>
        </box>
        <textarea
          onSubmit={() => {
            props.onConfirm?.({
          ...
          placeholder={t("dialog.export.filenamePlaceholder")}
      ...
      <box flexDirection="column">
        ...
        >
          ...
          <text fg={store.active === "thinking" ? theme.primary : theme.text}>
            {t("dialog.export.includeThinking")}
          </text>
        </box>
        <box
          flexDirection="row"
        ...
        >
          ...
          <text fg={store.active === "toolDetails" ? theme.primary : theme.text}>
            {t("dialog.export.includeToolDetails")}
          </text>
        </box>
        <box
          flexDirection="row"
        ...
        >
          ...
          <text fg={store.active === "assistantMetadata" ? theme.primary : theme.text}>
            {t("dialog.export.includeAssistantMetadata")}
          </text>
        </box>
        <box
          flexDirection="row"
        ...
        >
          ...
          <text fg={store.active === "openWithoutSaving" ? theme.primary : theme.text}>
            {t("dialog.export.openWithoutSaving")}
          </text>
        </box>
      </box>
      <Show when={store.active !== "filename"}>
        <text fg={theme.textMuted} paddingBottom={1}>
          {t("dialog.export.toggleHint", { toggle: "space", confirm: "return" })}
        </text>
      </Show>
      <Show when={store.active === "filename"}>
        <text fg={theme.textMuted} paddingBottom={1}>
          {t("dialog.export.filenameHint", { confirm: "return", options: "tab" })}
        </text>
      </Show>
    </box>
  )
