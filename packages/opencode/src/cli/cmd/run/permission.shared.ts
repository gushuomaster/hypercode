...
export function permissionInfo(request: PermissionRequest): PermissionInfo {
  ...
  if (request.permission === "external_directory") {
    ...
    return {
      icon: "←",
      title: t("permission.externalDirectory", { path: toolPath(dir, { home: true }) }),
  ...
  if (request.permission === "doom_loop") {
    return {
      ...
      lines: [t("permission.continueFailuresDescription")],
    }
  }

  return {
    icon: "⚙",
    title: t("permission.callTool", { tool: request.permission }),
    lines: [t("permission.tool", { tool: request.permission })],
  }
}

export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    return [t("permission.allowUntilRestart", { permission: request.permission })]
  }

  return [
    t("permission.allowPatterns"),
    ...request.always.map((item) => `- ${item}`),
  ]
}

export function permissionLabel(option: PermissionOption): string {
  if (option === "once") return t("permission.allowOnce")
  if (option === "always") return t("permission.allowAlways")
  if (option === "reject") return t("permission.reject")
  if (option === "confirm") return t("permission.confirm")
  return t("permission.cancel")
}
