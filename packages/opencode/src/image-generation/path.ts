...
export async function normalizeOutputPath(projectDirectory: string, outputPath: string) {
  const project = await fs.realpath(path.resolve(projectDirectory))
  const target = path.resolve(project, outputPath)
  if (!contains(project, target)) throw new Error("Image output path must remain inside the selected project directory")

  const ancestor = await nearestExistingPath(target)
  if (!contains(project, await fs.realpath(ancestor)))
    throw new Error("Image output path must not escape through a symbolic link")
  return target
}
