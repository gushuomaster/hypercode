...
HyperCode 默认启用 LSP 分析，但只在打开或处理上述匹配文件且满足相应要求时，按需启动对应服务器。

:::note
你可以将 `OPENCODE_DISABLE_LSP_DOWNLOAD` 环境变量设置为 `true` 来禁用 LSP 服务器的自动下载。该设置不会关闭 LSP 分析，本地已经可用的语言服务器仍可按需启动。HyperCode 离线交付默认设置该变量。
:::
...
HyperCode 省略 `lsp` 时默认启用内置 LSP 服务器。可将 `lsp` 设置为 `false` 来显式禁用所有 LSP 服务器：
