...
HyperCode enables LSP analysis by default. A server starts lazily only when a matching file is opened or processed and its requirements are met.

:::note
You can disable automatic LSP server downloads by setting the `OPENCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`. This does not disable LSP analysis; language servers already available locally can still start when needed. HyperCode offline distributions set this variable by default.
:::
...
If `lsp` is omitted, HyperCode enables the built-in LSP servers by default. Set `lsp` to `false` to explicitly disable all LSP servers:
