...
- HyperCode 当前维持无 license 门禁的通用版行为，并把这件事当作长期反回归边界。
...
### 5. 无 license 门禁的通用版行为

HyperCode 当前维持无 license 门禁的通用版行为，并把这件事视为反回归边界。
...
这不是临时性遗漏，而是当前产品定位的一部分。后续同步 upstream 时，如果相关区域再次出现授权门禁、授权入口或授权测试，就应被视为需要人工确认的回退信号。
...
- 看长期规则与禁止事项：`docs/规范`
- 看执行步骤与验收命令：`docs/操作手册`
- 看某次历史取证与验证现场：`docs/阶段记录`
