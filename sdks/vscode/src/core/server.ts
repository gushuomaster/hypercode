...
import { getCliPath, getHttpProxy } from "./settings"
...
export function resolveOpencodeCommand(configured: string) {
  ...
  return detectAvailableCommand(["hypercode", "opencode"]) ?? "hypercode"
}

export function shouldUseShell(command: string) {
...
function formatSpawnError(err: NodeJS.ErrnoException) {
  ...
  const command = resolveOpencodeCommand(configured)
  const hint = configured
    ? ` (configured via "hypercode.cliPath": ${configured})`
    : ' (set "hypercode.cliPath" to point at the HyperCode runtime if it lives outside PATH)'

  if (err.code === "ENOENT") {
    return `failed to start HyperCode runtime: command "${command}" was not found on the current host PATH${hint}`
  }

  if (err.code === "EACCES") {
    return `failed to start HyperCode runtime: command "${command}" is not executable on the current host${hint}`
  }
...
function detectAvailableCommand(candidates: string[]) {
  const checker = process.platform === "win32" ? "where" : "which"

  for (const candidate of candidates) {
    const result = cp.spawnSync(checker, [candidate], {
      shell: true,
      stdio: "ignore",
      windowsHide: true,
    })
