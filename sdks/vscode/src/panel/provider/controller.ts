...
export class SessionPanelController implements vscode.Disposable {
    ...
    this.panel.webview.onDidReceiveMessage(
      (message: WebviewMessage) => {
        ...
        if (message?.type === "navigateSession") {
          void vscode.commands.executeCommand("hypercode.openSessionById", this.ref, message.sessionID)
          return
        }

        if (message?.type === "newSession") {
          void vscode.commands.executeCommand("hypercode.newSessionAndOpen", this.ref)
          return
        }

        if (message?.type === "newSessionInPlace") {
          void vscode.commands.executeCommand("hypercode.newSessionInPlace", this.ref)
          return
        }
        ...
        if (message?.type === "switchSessionInPlace") {
          void vscode.commands.executeCommand("hypercode.switchSessionInPlace", this.ref, message.sessionID)
          return
        }

        if (message?.type === "sessionPickerAction") {
          void (async () => {
            await vscode.commands.executeCommand("hypercode.sessionPickerAction", this.ref, message.sessionID, message.action)
        ...
        if (message?.type === "openDocs") {
          if (message.target === "providers") {
            void vscode.commands.executeCommand("hypercode.openProviderDocs")
          }
          return
        }
        ...
        if (message?.type === "messageAction") {
          ...
          if (message.action === "forkUserMessage") {
            void vscode.commands.executeCommand("hypercode.forkSessionMessage", this.ref, message.messageID)
          }
          return
        }
    ...
    void build.deferred
      .then(async (deferred) => {
        ...
        const deferredPayload = {
          ...
          mcpStates: deferred.mcpStates,
          lspStates: deferred.lspStates,
          formatterStates: deferred.formatterStates,
          mcp: deferred.mcp,
  ...
  private async postSessionPicker() {
    const payload = await vscode.commands.executeCommand<Extract<HostMessage, { type: "sessionPicker" }>["payload"] | undefined>(
      "hypercode.getSessionPickerPayload",
      this.ref,
  ...
  private seedDeferredFields(snapshot: SessionSnapshot) {
    ...
    return patch({
      ...
      mcpStates: this.current.mcpStates,
      lspStates: this.current.lspStates,
      formatterStates: this.current.formatterStates,
