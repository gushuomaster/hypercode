...
export function sameSessionRef(
  left: Pick<SessionBootstrap["sessionRef"], "workspaceId" | "dir" | "sessionId">,
  right: Pick<SessionBootstrap["sessionRef"], "workspaceId" | "dir" | "sessionId">,
) {
  if (left.workspaceId && right.workspaceId) {
    return left.workspaceId === right.workspaceId && left.sessionId === right.sessionId
  }

  return left.dir === right.dir && left.sessionId === right.sessionId
}

export function resetSessionScopedComposerState(state: AppState): AppState {
  return {
    ...state,
    draft: "",
...
function samePersistedSession(initialRef: SessionBootstrap["sessionRef"] | null, persisted?: PersistedAppState) {
  ...
  return sameSessionRef(initialRef, {
