...
export function App() {
  ...
  const initialAgentOverrideResetRef = React.useRef(false)
  ...
  React.useEffect(() => {
    ...
    initialAgentOverrideResetRef.current = false
  ...
  React.useEffect(() => {
    document.title = `HyperCode: ${sessionTitle(state.bootstrap)}`
  }, [state.bootstrap])
  ...
  React.useEffect(() => {
    if (initialAgentOverrideResetRef.current) {
      return
    }
    if (state.bootstrap.status !== "ready") {
      return
    }
    if (state.snapshot.messages.length > 0) {
      initialAgentOverrideResetRef.current = true
      return
    }
    ...
    initialAgentOverrideResetRef.current = true
    setState((current) => {
      if (current.bootstrap.status !== "ready") {
        return current
      }
      if (current.snapshot.messages.length > 0 || !current.composerAgentOverride) {
        return current
      }
      return {
        ...current,
