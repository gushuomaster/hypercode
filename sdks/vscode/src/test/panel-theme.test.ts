...
describe("panel theme settings", () => {
  test("reads showSkillsInSlashAutocomplete from display settings", () => {
    ;(vscode.workspace as typeof vscode.workspace & {
      ...
      get: <T,>(key: string, fallback: T) => {
        if (section === "hypercode" && key === "showSkillsInSlashAutocomplete") {
          return true as T
        }
        return fallback
      },
  ...
  test("reads panelTheme from display settings", () => {
    ;(vscode.workspace as typeof vscode.workspace & {
      ...
      get: <T,>(key: string, fallback: T) => {
        if (section === "hypercode" && key === "panelTheme") {
          return "claude" as T
        }
        return fallback
      },
  ...
  test("reads panelColorScheme from display settings", () => {
    ;(vscode.workspace as typeof vscode.workspace & {
      ...
      get: <T,>(key: string, fallback: T) => {
        if (section === "hypercode" && key === "panelColorScheme") {
          return "orchid" as T
        }
        return fallback
      },
  ...
  test("treats panelTheme changes as display setting changes", () => {
    const event = {
      affectsConfiguration: (key: string) => key === "hypercode.panelTheme",
  ...
  test("treats panelColorScheme changes as display setting changes", () => {
    const event = {
      affectsConfiguration: (key: string) => key === "hypercode.panelColorScheme",
    } as vscode.ConfigurationChangeEvent
  ...
  test("treats showSkillsInSlashAutocomplete changes as display setting changes", () => {
    const event = {
      affectsConfiguration: (key: string) => key === "hypercode.showSkillsInSlashAutocomplete",
    } as vscode.ConfigurationChangeEvent
  ...
  test("offers classic instead of default for the HyperCode-native theme", () => {
    const items = buildThemePickerItems("classic", "default")
  ...
  test("does not declare opencode-web in the extension configuration enum", () => {
    ...
    const values = pkg.contributes?.configuration?.properties?.["hypercode.panelTheme"]?.enum ?? []
  ...
  test("does not declare default in the extension configuration enum", () => {
    ...
    const panelTheme = pkg.contributes?.configuration?.properties?.["hypercode.panelTheme"]
  ...
  test("declares panel color schemes in the extension configuration enum", () => {
    ...
    const panelColorScheme = pkg.contributes?.configuration?.properties?.["hypercode.panelColorScheme"]
